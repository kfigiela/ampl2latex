...
trait LogicalExpressionAMPLParser extends JavaTokenParsers {
  ...
  private def not: Parser[LogicalExpression] =
    ("!" | "not") ~> nonRecursiveLogicalProductionsParser ^^ {
      case l: LogicalExpression => Logical.not(l)
    }

  private def memberInclusion: Parser[LogicalExpression] =
    member ~ "in" ~ sexpr ^^ {
      case m ~ _ ~ (s: SetExpression) => Inclusion.member(m, s)
    }

  private def subsetInclusion: Parser[LogicalExpression] =
    sexpr ~ "within" ~ sexpr ^^ {
      case (s1: SetExpression) ~ _ ~ (s2: SetExpression) => Inclusion.subset(s1, s2)
    }

  private def memberExclusion: Parser[LogicalExpression] =
    member ~ "not" ~ "in" ~ sexpr ^^ {
      case m ~ _ ~ _ ~ (s: SetExpression) => Exclusion.member(m, s)
    }

  private def subsetExclusion: Parser[LogicalExpression] =
    sexpr ~ "not" ~ "within" ~ sexpr ^^ {
      case (s1: SetExpression) ~ _ ~ _ ~ (s2: SetExpression) => Exclusion.subset(s1, s2)
    }

  private def parenthesized: Parser[LogicalExpression] =
    "(" ~> lexpr <~ ")" ^^ {
      case l: LogicalExpression => ParenthesizedLogical(l)
    }
