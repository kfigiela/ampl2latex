...
class NodeMapper(val operations: mutable.Seq[NodeMapper]) {
  ...
  def andThen[B](v: TmpVisitor[Declaration, B]): NodeAggregator[B] = new NodeAggregator[B](operations :+ this, v)

  def map(node: Declaration): Declaration = node match {
    case Assertion(index, lexpr) =>
      Assertion(index.map(mapIndexing), mapLexpr(lexpr))
    case Assertion(None, lexpr)  =>
      Assertion(None, mapLexpr(lexpr))

    case ConstraintDeclaration(n, a, index, constr) =>
      ConstraintDeclaration(mapName(n), mapAlias(a), index
        .map(mapIndexing), mapConstraint(constr))
  ...
  def mapExpr(e: Expression): Expression = e match {
    case ParenthesizedExpression(expr) => ParenthesizedExpression(mapExpr(expr))

    case ExpressionIf(cond, t, f) => ExpressionIf(mapLexpr(cond), mapExpr(t), mapExpr(f))

    case FunctionCall(name, args) =>
      FunctionCall(mapName(name), args.map(mapExpr))

    case PiecewiseLinearTerm(optExpres, optIndexes, (expr, cexpr)) =>
      PiecewiseLinearTerm(optExpres.map(b => (b._1.map(mapIndexing), mapExpr(b
        ._2))),
        optIndexes.map(s => (s._1.map(mapIndexing), mapExpr(s._2))),
  ...
  def mapSexpr(s: SetExpression): SetExpression = s match {
    case ParenthesizedSetExpression(sexpr) => ParenthesizedSetExpression(mapSexpr(sexpr))

    case SetExpressionIf(cond, t, f) => SetExpressionIf(mapLexpr(cond), mapSexpr(t), mapSexpr(f))
    case IndexedSet(is, sexpr)       => IndexedSet(mapIndexes(is), mapSexpr(sexpr))
  ...
  def mapBoundedConstraint(bounded: BoundedConstraint): BoundedConstraint = bounded match {
    case BoundedConstraint(left, expr, right) =>
      BoundedConstraint(left.map(mapConstraintComparision), mapExpr(expr),
        right.map(mapConstraintComparision))
