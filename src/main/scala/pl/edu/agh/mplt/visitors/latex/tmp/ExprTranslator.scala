...
import pl.edu.agh.mplt.visitors.latex.mappers.{AddNecessaryParenthesis, StripAllParenthesis}
...
class ExprTranslator extends Translator[Expression] {
  ...
  override def apply(node: Expression): String = node match {
    ...
    case a: ArithmeticOperation         => (new StripAllParenthesis andThen new AddNecessaryParenthesis andThen new ArithmeticTranslator)(a)

    case e => s"Unsupported Expression: $e"
  }
